            ...
            SELECT procedures.name, 
					cast(
						concat(CAST(
								...
								DATE_PART('hour', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp) AS CHAR(2)
								), 
					   		':',
					  		   CAST(
									DATE_PART('minute', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp) AS CHAR(2)
								),
					  		':',
					   			CAST(
									DATE_PART('second', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp) AS CHAR(2)
					  			)
					  		)
						AS varchar(8))
				 as average_duration,
                ...
                WHERE 
                 (
                    DATE_PART('day', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp)
                    + (DATE_PART('hour', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp))
                    + (DATE_PART('minute', events.surgeon_ended_at::timestamp - events.surgeon_started_at::timestamp))

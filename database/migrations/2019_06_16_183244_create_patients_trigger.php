...
class CreatePatientsTrigger extends Migration
{
    /**
     * Run the migrations.
     *
        ...
        DB::statement("
            ...
            BEGIN
                ...
                IF(countPatients > 0) THEN
                    RAISE EXCEPTION 'The given medical record is already registered!';
                    RETURN NULL;
                END IF;
                ...
                ELSIF (NEW.birthday_at IS NULL) THEN
                    RAISE EXCEPTION 'The \"birthday_at\" attribute is required!';
                    RETURN NULL;
                ELSIF (NEW.mother_name IS NULL) THEN
                    RAISE EXCEPTION 'The \"mother_name\"  attribute is required!';
                    RETURN NULL;
                ELSIF (NEW.gender IS NULL) THEN
                    RAISE EXCEPTION 'The \"gender\" attribute is required!';
                    RETURN NULL;
                ELSIF (NEW.medical_record IS NULL) THEN
                    RAISE EXCEPTION 'The \"medical_record\" attribute is required!';
                    RETURN NULL;
                END IF;
                -- Check for minstring length
                IF(SELECT LENGTH(NEW.name) NOT BETWEEN 1 AND 70)THEN
                    RAISE EXCEPTION 'The \"name\" must have between 1 and 70 characters';
                    RETURN NULL;
                ELSIF(SELECT LENGTH(NEW.mother_name) NOT BETWEEN 1 AND 70) THEN
                    RAISE EXCEPTION 'The \"mother_name\" attribute must have between 1 and 70 characters';
                    RETURN NULL;
                ELSIF (SELECT LENGTH(NEW.medical_record) NOT BETWEEN 1 AND 15) THEN
                    RAISE EXCEPTION 'The \"medical_record\" attribute must have between 1 and 15 characters';
                    RETURN NULL;
                END IF;
                
                -- Check for gender in gender enum:
                IF((NEW.gender <> 'M') AND (NEW.gender <> 'F') AND (NEW.gender <> 'O')) THEN
                    RAISE EXCEPTION 'The \"gender\" attribute must be one of the following characters: \"M\", \"F\", \"O\"';
                    RETURN NULL;
                END IF;
                RETURN NEW;
            END;
    ...
    public function down()
    {
        ...
        DB::statement("DROP PROCEDURE IF EXISTS verify_patient_constraints_for_update()");
        DB::statement("DROP TRIGGER IF EXISTS verify_update_patients_contraints_on_patients_for_update ON patients");
    }
}
